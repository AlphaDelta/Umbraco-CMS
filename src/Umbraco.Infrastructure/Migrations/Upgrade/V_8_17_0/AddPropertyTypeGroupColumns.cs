...
namespace Umbraco.Cms.Infrastructure.Migrations.Upgrade.V_8_17_0
{
    ...
    {
        private readonly ILogger<AddPropertyTypeGroupColumns> _logger;
        private readonly IShortStringHelper _shortStringHelper;

        public AddPropertyTypeGroupColumns(IMigrationContext context, ILogger<AddPropertyTypeGroupColumns> logger, IShortStringHelper shortStringHelper)
            : base(context)
        {
            _logger = logger;
        ...
        internal IEnumerable<PropertyTypeGroupDto> PopulateAliases(IEnumerable<PropertyTypeGroupDto> dtos)
        {
            foreach (var dtosPerAlias in dtos.GroupBy(x => x.Text.ToSafeAlias(_shortStringHelper, true)))
            {
                ...
                if (numberSuffix > 2)
                {
                    _logger.LogError("Detected the same alias {Alias} for different property group names {Names}, the migration added suffixes, but this might break backwards compatibility.", dtosPerAlias.Key, dtosPerAliasAndText.Select(x => x.Key));
                }
            }
        }
    }
